...
# Genera un unico .exe autocontenido para Windows x64
...
# Verificar que el SDK esta disponible
...
if ($LASTEXITCODE -eq 0) {
    Write-Host ''
    Write-Host ('Listo! Ejecutable generado en: {0}\EjecutableNFC.exe' -f $output) -ForegroundColor Green
    Write-Host ''
    # Abrir la carpeta en el Explorador
    Start-Process explorer.exe $output
} else {
    Write-Host ''
    Write-Host 'ERROR: La compilacion fallo con codigo ' -NoNewline
